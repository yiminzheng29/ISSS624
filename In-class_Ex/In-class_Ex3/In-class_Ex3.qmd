...
### 7.1 Extrating clustering variables
...
```{r}
shan.w <- nb2listw(shan.nb,  #to make it into weight matrix
                   lcosts, 
                   style="B")
summary(shan.w)
```
...
clust6 <- skater(edges = shan.mst[,1:2], 
                 data = shan_ict, 
                 method = "euclidean", 
                 ncuts = 5) # at cut 5, there are 6 clusters
```
...
shan_sf_spatialcluster <- cbind(shan_sf_cluster, as.factor(groups_mat)) %>% #better to use simple feature here
...
### 9.1 Ward-like hierarchical clustering: ClustGeo (only works with ward method)
...
```{r}
dist <- st_distance(shan_sf, shan_sf) #st_distance is more flexible
...
#proxmat(d0) -> attribute matrix
#distmat(d1) -> geographical matrix
#first graph -> based on raw
#second graph -> based on normalized values
#for 2nd graph, the best points are 0.2, or 0.3. at 0.2, spatial homogenity is 75%, but 20% is lost in attribute homogeneity. So in the next code, we choose alpha = 0.3
```

With reference to the graphs above, alpha = 0.3 will be used as shown in the code chunk below.

```{r}
clustG <- hclustgeo(proxmat, distmat, alpha = 0.3) #0.3 is based on the cut off value in the 2nd chart. You're increasing the spatial homogeneity in this case. 
```
