...
This exercise will show how you can compute spatial weights using R.
...
### 1.2 Importing csv file into r environment

Next, the *Hunan_2012.csv* file will be imported into R by using *read_csv()* of **readr** package. The output is R dataframe class.
...
*\--\> It measures that on average, the number of neighbours satisfying the distance criteria defined in the above formula. This would mean the number of neighbours of region points by Euclidean distance in the metric that fulfills the lower bound of 0 and the upper bounds of 62, with longlat as TRUE.*
...
```{r}
lag.list.knn6a <- list(hunan$NAME_3, lag.listw(rswm_q, hunan$GDPPC))
