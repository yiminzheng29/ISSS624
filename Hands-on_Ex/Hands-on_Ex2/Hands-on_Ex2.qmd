...
### 2.3 Setting up the Analytical Tools
...
>
> With a low p-value of almost 0 and a Moran I statistic which is \>0, it is likely that the values in the data are clustered, and observations tend to be similar.
...
> Question: What statistical conclusion can you draw from the output above?
>
> Similar to the conclusion above, the p-value is low (0.001) is unsurprising and further validates the fact that the values in the data are strongly clustered.
...
>
> The histogram shows that the distribution of Moran I values possibly expected if the observations have been arranged in randomness. However, the observed statistic of 0.30075 falls way to the right of the distribution, suggesting that positive spatial autocorrelation is occurring and the values are clustered.

> Question: Instead of using Base Graph to plot the values, plot the values by using ggplot2 package

```{r}
bperm1 = as.data.frame(bperm$res) # change values into dataframe before ggplot is called
...
> Question: What statistical conclusion can you draw from the output above?
>
> The Geary C statistic value of 0.6907223 (small c value since it is less than 1) shows that the values are clustered and the observations tend to be similar.
...
> Question: What statistical conclusion can you draw from the output above?
>
> A Geary C statistic value of 0.69072 similarly suggests that there is positive spatial autocorrelation since the value is lower than 1.
...
> Question: What statistical conclusion can you draw from the output above?
>
> The Geary C's statistic value of 0.6907 is to the way left of the distribution in the histogram, suggesting that positive spatial autocorrelation is occurring and the values are clustered.
...
>
> At lag distance 1-4, there is positive spatial autocorrelation since values of Moran's I estimates are positive. This thus indicates that the values are clustered. Whereas when the lag distance increases to 5 or 6, the Moran's I estimates become negative and it thus indicates a negative spatial autocorrelation, and values are likely to be dispersed.
...
>
> The high-high clusters indicate positive autocorrelation for the county areas having adjacent high GDPPC values. This is similar for the low-low clusters which indicate positive autocorrelation for county areas having adjacent low values.
>
> The low-high clusters indicate negative correlation where these areas with low GDPPC values are surrounded by high GDPPC areas. This is also similar to the high-low clusters which indicate negative correlation where areas with high GDPPC values are surrounded by low GDPPC areas.
...
>
> The east and west regions of the Hunan province showed a clear divide in the choropleth map above. Areas shaded in pink/red represented areas (hot spot areas) with higher GDPPC whilst areas shaded in blue/pale blue (cold spot areas) seemed to be areas/counties with lower GDPPC.
>
> Citizens with higher GDPPC apparently resided in the eastern side of Hunan, whilst citizens with lower GDPPC resided in the western side of Hunan.
...
> Question: What statistical observation can you draw from the Gi map above?
>
> Similar to the fixed weights results above, the map showed a even clearer divide between the east and west counties of the Hunan province. The east side of Hunan is covered with more pink/red areas (hot spot areas) as compared to the the Gi map under fixed weights. The west side areas had more pale blue colored regions (cold spot areas) than as compared to the Gi map plotted under fixed weights.
>
> This validates the fact that citizens residing in the east side of the Hunan province had a distincively higher GDPPC than as opposed to residents in the west regions of the Hunan province.
