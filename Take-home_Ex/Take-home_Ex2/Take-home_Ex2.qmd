...
We can see from the heatmap above, most of the LGAs are rural areas (as seen from its deep shade of blue occupying most of the plots). Different clusters have different characteristics. For instance, the blue cluster has a high percentage of functional water points, as well as a high percentage of hand pumps. We will continue further analyses below to observe the different attributes in each cluster.
...
**Cluster 3** has the highest count in all its variables as seen from the cluster summary plot. It has one of the least percentage of usage of at least 1,000. This suggests that its water points are not as heavily utilised than as compared to LGAs in Cluster 1 and 2.

**Cluster 4** performed the best in mean percentage of functional water points. It also has a relatively low percentage of non-functional water points and high mean percentage of hand pumps. This suggests also that the water points are relatively well maintained in this cluster.
...
km.map <- qtm(nigeria_kmeans,fill = "KM_Cluster")+ 
  tm_borders(alpha = 0.5) +tm_layout(main.title = "K-means method", main.title.position = "center")
...
**Clusters 2 and 3** both had relatively low mean percentage usage of 1,000, as seen from its multivariate visualisations. This suggests low utilisation of its water points in these 2 clusters. However, **cluster 3** has higher mean percentage of functional water points and hand pumps than **cluster 2**. This suggests that the **cluster 2**'s water points are better maintained than **cluster 3**.

**Cluster 4** had the highest number of LGAs, as seen from the cluster summary plot and its choropleth map. However, it fared the lowest in the mean percentage of functional water points. These LGAs are mostly situated around the borders of Nigeria and the south regions.

**Cluster 5** has higher mean percentage of functional water points than **cluster 4**. The water points are also less utilised, as seen from its lower mean percentage usage of at least 1,000.
...
Cluster 1 and 5 were relatively large clusters, whilst 2,3 and 4 were relatively smaller in comparison.
...
From the output above, we can see that cluster 4 has the highest mean percentage of functional water points, whilst Cluster 2 had the lowest.
...
From the output above, we can see that **cluster 4** have relatively well maintained water points, since it has the highest mean percentage of functional water points and lowest mean percentage of non-functional water points. It also has a relatively high mean percentage of hand pump. Meanwhile, **cluster 1, 2 and 3** may be areas of concern as it has one of the highest mean percentage of non-functional water points. **Cluster 5** has the lowest mean percentage usage of at least 1000, which suggests that the water points in this cluster are not as highly utilised than as compared to the other clusters.
...
cg.map <- qtm(nigeria_sf_Gcluster, "CLUSTER") + 
  tm_borders(alpha = 0.5) +tm_layout(main.title = "ClustGeo method", main.title.position = "center")
...
We can see from the outputs above that, **cluster 4** has relatively well-maintained water points. It has the highest mean percentage of functional water points and hand pumps, and also lowest mean percentage for its non-functional water points and percentage usage. **Clusters 2 and 5** also have relatively high mean percentage of functional water points as well. There are however, less LGAs in **clusters 4** and **5**.

**Clusters 1 and 3** fared the lowest in mean percentage of functional water points. They also have higher mean percentage of non-functional water points in comparison to the other clusters. However, there are many LGAs in these 2 clusters. These may be areas of concerns where the government may want to look into to improve the functionality of the water points.
...
As we can see, all the methods above have its benefits and limitations of performing cluster analysis. With a side by side comparison, it seems obvious that the conventional hierarchical clustering method, the k-means clustering method, as well as the ClustGeo method, produced relatively similar looking choropleth maps. The SKATER method did not produce a similar map than as compared to the other maps and clearly, it may be less wise to use the SKATER method for our cluster analysis here, as the LGAs may not be spatially contiguous based on the variables that we are analysing in this exercise.
