...
In this exercise, we will use geospatial analytics to delineate homogeneous regions by using geographically referenced multivariate data. Four analysis will be performed; namely, the conventional hierarchical cluster analysis, the k-means clustering, and the spatially constrained cluster analysis, using the SKATER method and the ClustGeo method. We will be looking into ***Nigeria's*** data for the purpose of this study, and the below packages will be used for analysing and visualising the data:
...
-   `kmeans()` of **cluster** package will be used for k-means cluster analysis

-   the **ClustGeo** package will be used to perform cluster analysis, using `hclust()`*.*

-   `skater()` of Base R will be used to perform spatially constrained cluster analysis.
...
Note: The code chunks are all folded in order to have better visibility on the site. To enable viewing of [all]{.underline} the code chunks, please toggle the option on the top right hand corner to "Show All Code".

![](images/Screenshot%202022-12-13%20at%204.57.57%20PM.png){width="95"}

## 2. Getting Started

### 2.1 The analytical question

In geobusiness and spatial policy, it is common to delineate the planning area into homogeneous regions, using multivariate data. And hence, in this exercise, we plan to delineate the LGAs of Nigeria, into homogeneous regions based on various attributes of its water points (e.g. functional / hand-pumps and etc).

![](images/paste-3D3DCF40.png){width="604"}

Above is the map of Nigeria, segregated into different regions.
...
We will use `filter()` of **dplyr** to select [functional]{.underline} water points.
...
Similarly, we will use `filter()` of **dplyr** to select [non-functional]{.underline} water points.
...
Now, we will use `filter()` of **dplyr** to select [unknown]{.underline} water points.
...
Next, we will use `filter()` of **dplyr** to extract data with water points of usage capacity that is less than 1000, based on the *usage_cap* column.
...
Now, we will also use `filter()` of **dplyr** to extract data with water points of usage capacity of at least 1000, based on the *usage_cap* column.
...
Now, we will use `filter()` of **dplyr** to extract data with water points in rural areas. This information can be retrieved from *is_urban* column in the data.
...
Now that we are done with the cleaning of names, we will also remove LGAs with zero water points as they are irrelevant to our analysis.

### 3.6 Removing LGAs with zero water points

```{r}
#| code-fold: true
#| code-summary: "Show the code"
nigeria_derived<- nigeria_derived[nigeria_derived$`total wpt` != 0, ]
```

Now, we have removed LGAs with zero water points.

### 3.7 Transforming data frame to Nigeria's Projected Coordinate System

We will run the code chunk below to transform the data frame to Nigeria's projected coordinate system, which is, EPSG 26391.

```{r}
#| code-fold: true
#| code-summary: "Show the code"
nigeria_derived <- nigeria_derived %>% st_transform(crs = 26391)
```

Let's take a quick glance at the newly transformed data.

```{r}
#| code-fold: true
#| code-summary: "Show the code"
glimpse(nigeria_derived)
```

From the data above, there are 761 LGAs left for analysis, which is down from the original 774 LGAs.

### 3.7 Replacing NA values with 0

We will now execute the code chunk below to replace NA values with 0. This will prevent any potential errors in later steps.
...
Now, we are ready to perform Exploratory Data Analysis (EDA) in the next section.
...
Both the histogram and boxplot showed a more normal distribution, with less outliers, if percentage of functional water points was used instead of the count of functional water points.
...
#| code-fold: true
#| code-summary: "Show the code"
fnc <- ggplot(data=nigeria_derived, 
             aes(x= `wpt functional`)) +
  geom_histogram(bins=20, 
                 color="black", 
                 fill="light green")

non_fnc <- ggplot(data=nigeria_derived, 
...
ggarrange(fnc, non_fnc, pct_fnc, pct_non_fnc, pct_hp, pct_l1000, pct_al1000, pct_rural, 
          ncol = 4, 
          nrow = 2)
```
...
#| code-fold: true
#| code-summary: "Show the code"
fnc_bp <- ggplot(data=nigeria_derived, 
             aes(x= `wpt functional`)) +
  geom_boxplot(color="black", 
                 fill="light green")

non_fnc_bp <- ggplot(data=nigeria_derived, 
             aes(x= `wpt non-functional`)) +
  geom_boxplot(color="black", 
                 fill="light green")
...
ggarrange(fnc_bp, non_fnc_bp, pct_fnc_bp, pct_non_fnc_bp, pct_hp_bp, pct_l1000_bp, pct_al1000_bp, pct_rural_bp, 
          ncol = 4, 
          nrow = 2)
```

As we can see, there are many outliers for *wpt functional* and *wpt non-functional*. This may be due to some LGAs having significantly more functional/non-functional water points.
...
We will first set "plot" for the creation of choropleth maps.
...
To reveal if the distribution shown in the choropleth map above are bias to underlying total number of water points at LGA levels, we will create two choropleth maps, one for the total number of water points, and another for the total number of functional water points. The code chunk below is used to display the output.
...
We can see that, out of the states with water points, most of them have functional water points at the North east and North west regions. It may indicate that these areas are more urban and hence, there are more functional water points. We will perform further analysis in the below steps to support our speculations here.

One interesting point to note here is that, quite a number of LGAs in the Borno state has disappeared from the map. This is because the LGAs did not have any water points, and were hence removed in step 3.6. Borno is actually the state that is worst-affected by the humanitarian crisis, and as a result, they did not have much water points available, which thus explains some of the LGA's disappearance from the choropleth maps above.
...
It is interesting to note that although some areas have very little water points, they can be very high in percentage of functionality.
...
cluster_vars <- nigeria_derived %>%
  ...
  select("wpt functional", "wpt non-functional", "pct_functional", "pct_non_functional", "pct_usage_below_1000", "pct_usage_at_least_1000", "pct_rural", "pct_hand_pump")
```
...
In this section, we will perform hierarchical cluster analysis. The analysis consists of four major steps, which are as laid out in points 6.1, 6.2, 6.3 and 6.4:

### 6.1 Extracting clustering variables

Due to the high correlation between certain variables as mentioned in point 5, we will be dropping *pct_usage_below_1000* from the data frame. The remaining 7 computed variables - *wpt functional, wpt non-functional,* *pct_functional*, *pct_non_functional*, *pct_usage_at_least_1000*, *pct_rural* and *pct_hand_pump* will be extracted to the new data frame called *cluster_vars_new*.
...
cluster_vars_new <- nigeria_derived %>%
  st_set_geometry(NULL) %>%
  select("wpt functional", "wpt non-functional", "pct_functional", "pct_non_functional", "pct_usage_at_least_1000", "pct_rural", "pct_hand_pump")
```
...
We will now proceed to remove the shapeName column using the code chunk below, to ensure that the LGA names are set as index for the data frame.

```{r}
#| code-fold: true
#| code-summary: "Show the code"
nigeria_ict <- select(cluster_vars_new, c(1:7))
head(nigeria_ict, 10)
```

### 6.2 Data Standardisation

Multiple variables will be used in this cluster analysis. In order to avoid the cluster analysis being biased to clustering variables with large values, we will first standardise the input variables, before performing cluster analysis.
...
The results from Min-Max standardisation and Raw values without standardisation are almost the same, which is unsurprising. This is due to the variables being in percentage values ranging from 0% to 100%, and hence with min-max standardisation and the x-axis in percentages, the histograms for raw-values and Min-max standardisation will not show much difference.

Meanwhile, the Z-score standardisation showed subtle differences as compared to the other two histograms. The three histograms above also seemed relatively normally distributed in general.
...
We will use the code chunk below to compute the proximity matrix using the [euclidean]{.underline} method.
...
We will now determine the optimal number of clusters by using the three different methods proposed above, for learning purposes.

#### 6.7.3 Elbow Method

We will use `fviz_nbclust()` to plot the graph and determine optimal number of clusters.

```{r}
#| code-fold: true
#| code-summary: "Show the code"
fviz_nbclust(nigeria_ict, kmeans, method = "wss")
```

From the elbow method above, we can see that the optimal number of clusters suggested would be 3, as that is where the bend is observed and the decrease is slowing down.

#### 6.7.2 Silhouette Method

Again, we will be using the *scaled_data* to determine optimal number of clusters, using the silhouette method.

```{r}
#| code-fold: true
#| code-summary: "Show the code"
fviz_nbclust(nigeria_ict, kmeans, method='silhouette')
```

The silhouette method shows an optimal number of 2 clusters.

#### 6.7.3 Gap Statistic Method
...
\
The map above shows that 1 is the best number of clusters. However, as 1 is too low to be used as an optimal number for analysis, and hence we will use the next best value, which is 5.

As all three methods yield different results, we will be using the results from the gap statistic method to determine the optimal number of clusters (i.e. 5) below.
...
## 7. Conventional hierarchical clustering analysis
...
### 7.1 Transforming the data frame into a matrix
...
### 7.2 Plotting interactive cluster heatmap using `heatmaply()`
...
We can see from the heatmap above, most of the LGAs are rural areas (as seen from its deep shade of blue occupying most of the plots). Different clusters have different characteristics. For instance, the purple cluster has a high percentage of functional water points, as well as a high percentage of hand pumps. We will continue further analyses below to observe the different attributes in each cluster.

### 7.3 Mapping the clusters formed
...
Next, we will use `qtm()` of the **tmap** package to plot the choropleth map, in order to show the clusters.
...
As we can see, the clusters are very fragmented. This may be a major limitation when non-spatial clustering algorithm like hierarchical clustering analysis, is used.
...
#### 7.4.1 Visualising individual clustering variables
...
#| code-fold: true
#| code-summary: "Show the code"
ggplot(data=nigeria_sf_cluster,
       aes(X=CLUSTER, y=pct_functional)) + geom_boxplot()
```

We can see from the plot above that cluster 5 has the highest mean percentage of functional water points, whilst cluster 2 had the lowest mean percentage of functional water points.

#### 7.4.2 Plotting the cluster summary
...
There are some interesting observations from the cluster summary plot above. We will also perform multivariate visualisation on the mean values of the variables in each cluster for better analysis. This will be done using `ggparcood()` in the next step.

#### 7.4.3 Multivariate visualisation

```{r}
#| code-fold: true
#| code-summary: "Show the code"
ggparcoord(data = cluster_summary, 
           columns = c(2:6), 
           scale = "globalminmax",
           alphaLines = 0.2,
           boxplot = TRUE, 
...
**Cluster 1** has the lowest mean percentage of hand pumps and rural areas. However, it still has a relatively decent mean percentage of functional water points in comparison to other clusters. The mean percentage of non-functional water points is relatively low in cluster 1 as well. These observations suggest that Cluster 1 may consists of more urban areas and their water points are relatively well maintained.

The water points in **Cluster 2** seems to be limited and overused. As seen from the cluster summary plot, it has low counts of percentage of functional water points and low counts of percentage of hand pumps. The multivariate visualisation above also suggests that the mean percentage of non-functional water points and mean percentage of percentage usage of at least 1,000 is also relatively quite high. This cluster may be an area of concern for the government to improve on the functionality of its water pumps or increase the number of water pumps in order to ensure that each water point on average is not over utilised.

**Cluster 3** has the highest count in all its variables as seen from the cluster summary plot. It has one of the lowest mean percentage of usage of at least 1,000, as seen from the multivariate visualisation. This suggests that its water points are not as heavily utilised than as compared to LGAs in Cluster 1 and 2.

**Cluster 4** has a relatively low percentage of non-functional water points and high mean percentage of hand pumps. It's mean percentage of usage of at least 1,000 is one of the lowest. This suggests also that the water points are relatively well maintained/less utilised in this cluster.

As we can see, even though in the cluster summary plot, **Cluster 5** has the lowest counts in all its variables, it has the highest the highest mean percentage of functional water points, coupled with high percentage of hand pumps. **Cluster 5** probably are LGAs that are mostly rural but with relatively well maintained water points.

## 8. K-means clustering method

We will also attempt to use k-means clustering analysis to learn more about the characteristics of various clusters in Nigeria.

### 8.1 Computing the k-means cluster

Since we have computed the most optimal number of clusters above, we can use it to create a k-means clustering map, using the `kmeans()` function and the `fviz_cluster()` function. We will also use a nstart value of 25 as default.

```{r}
#| code-fold: true
#| code-summary: "Show the code"
set.seed(1)
km <- kmeans(nigeria_ict, centers = 5, nstart = 25)
fviz_cluster(km, data = nigeria_ict)
```

Now, we will use the `aggregate()` function to see the mean values of its attributes in different clusters.

```{r}
#| code-fold: true
#| code-summary: "Show the code"
aggregate(nigeria_ict, by=list(cluster=km$cluster), mean)
```

### 8.2 Plotting the choropleth map

We will also plot a choropleth map using k-means clustering for better visualisation.

```{r message=FALSE, warning=FALSE}
#| code-fold: true
#| code-summary: "Show the code"
nigeria_kmeans <- nigeria_derived %>%
  mutate(KM_Cluster = as.factor(km$cluster))
```

```{r message=FALSE, warning=FALSE}
#| code-fold: true
#| code-summary: "Show the code"
km.map <- qtm(nigeria_kmeans,fill = "KM_Cluster")+ 
  tm_borders(alpha = 0.5) +tm_layout(main.title = "K-means method", 
...
### 8.3 Visualisation Interpretation of clusters

Three methods will be performed below for the visual interpretation.

#### 8.3.1 Visualising individual clustering variables
...
#| code-fold: true
#| code-summary: "Show the code"
ggplot(data=nigeria_kmeans,
       aes(X=KM_Cluster, y=pct_functional)) + geom_boxplot()
```

**Cluster 1** has the highest mean percentage of functional water points, whilst **cluster 4** has the lowest mean percentage of functional water points. However, **cluster 1**'s range is the lowest, as seen from the length of its box.

#### 8.3.2 Plotting the cluster summary

```{r}
#| code-fold: true
#| code-summary: "Show the code"
cluster_summary_km <- nigeria_kmeans %>% st_set_geometry(NULL) %>% group_by(KM_Cluster) %>% summarise(pct_functional, pct_non_functional, pct_hand_pump, pct_usage_at_least_1000, pct_rural)
```

```{r}
#| code-fold: true
#| code-summary: "Show the code"
cluster_summary_km.molten <- melt(cluster_summary_km, na.rm=TRUE)
```

```{r}
#| code-fold: true
#| code-summary: "Show the code"
cluster_summary_plot_km <-
  ggplot(cluster_summary_km.molten, aes(variable, value, fill = variable)) + 
...
#### 8.3.3 Multivariate Visualisation

We will now use the `ggparcoord()` to perform multivariate visualisation of its clusters.

```{r}
#| code-fold: true
#| code-summary: "Show the code"
ggparcoord(data = cluster_summary_km, 
           columns = c(2:6), 
           scale = "globalminmax",
           alphaLines = 0.2,
           boxplot = TRUE, 
...
The below observations were made from the cluster summary plot and multivariate visualisations:

**Cluster 1** consists of the highest mean percentage of functional water points, hand pumps and rural, as seen from the multivariate visualisations. It also has the lowest percentage of non-functional water points and percentage of hand pumps, which suggests that the water points in this cluster are relatively well maintained. However, as seen from the cluster summary plot, not many LGAs belong to this cluster.

**Clusters 2 and 3** both had relatively low mean percentage usage of 1,000, as seen from its multivariate visualisations. This suggests low utilisation of its water points in these 2 clusters. However, **cluster 3** has higher mean percentage of functional water points and hand pumps than **cluster 2**. This may suggest that the **cluster 3**'s water points are better maintained than **cluster 2**.

**Cluster 4** had the highest number of LGAs, as seen from the cluster summary plot and its choropleth map. However, it fared the lowest in the mean percentage of functional water points. These LGAs are mostly situated around the borders of Nigeria and the south regions. They may be areas of concern where the Government may want to improve the functionality of its water points.

**Cluster 5** has a higher mean percentage of functional water points than **cluster 4**. The water points are also less utilised, as seen from its lower mean percentage usage of at least 1,000.
...
### 9.1 Converting into SpatialPolygonsDataFrame
...
Each observation will give the pairwise dissimilarity between its variables, based on the seven variables, and the values of its neighbouring observations from the neighbour list.
...
### 9.4 Computing minimum spanning tree
...
We can see that the dimension has been reduced to 760, due to the fact that the minimum spanning tree consists of n-1 edges (links) to traverse all the nodes.
...
### 9.5 Computing spatially constrained clusters using SKATER method
...
#| code-fold: true
#| code-summary: "Show the code"
clust5 <- spdep::skater(edges = nigeria.mst[,1:2], 
                 data = nigeria_ict, 
                 method = "euclidean", 
                 ncuts = 4)
```
...
-   number of cuts - this has been set to [one]{.underline} less than the optimal number of clusters (i.e. 3)
...
### 9.6 Visualising the clusters in a choropleth map
...
Cluster 1 is the largest cluster, followed by 4,2,3 and 5.
...
We can see the spatially constrained hierarchical clustering map seemed to provide a better visualisation than as compared to the hierarchical clustering map as the clusters are less fragmented. Now, we will take a look at each of the attributes within the SKATER clustering method for each of the 5 clusters.

### 9.7 Visual Interpretation of clusters

#### 9.7.1 Visualising individual clustering variables
...
#| code-fold: true
#| code-summary: "Show the code"
ggplot(data=nigeria_sf_spatialcluster,
       aes(X=SP_CLUSTER, y=pct_functional)) + geom_boxplot()
```

From the output above, we can see that **cluster 3** has the highest mean percentage of functional water points, whilst **Cluster 2** had the lowest.

#### 9.7.2 Plotting the cluster summary

Now, let's take a look at the cluster summary plot for the spatially constrained hierarchical clustering map.
...
#### 9.7.3 Multivariate Visualisation
...
#| code-fold: true
#| code-summary: "Show the code"
ggparcoord(data = cluster_summary_SKATER, 
           columns = c(2:6), 
           scale = "globalminmax",
           alphaLines = 0.2,
           boxplot = TRUE, 
...
From the output above, we can see that **cluster 3** have relatively well maintained water points, since it has the highest mean percentage of functional water points and lowest mean percentage of non-functional water points. It also has a relatively high mean percentage of hand pump. Cluster 3 also performed relatively well as it has a relatively high mean percentage of functional water points and low mean percentage of non-functional water points. Meanwhile, **cluster 1 and 2** may be areas of concern as it has one of the highest mean percentage of non-functional water points.

**Cluster 4** also seems to have relatively well maintained clusters, as seen from its high mean percentage of functional water points and low mean percentage of non-functional water points. It's percentage usage of at least 1,000 is also relatively low, which suggests that the water points are not very over-utilised.

**Cluster 5** has the lowest mean percentage usage of at least 1000, and it has a relatively high mean percentage of functional water points, which suggests that the water points in this cluster are not as highly utilised than as compared to the other clusters.
...
rect.hclust(nongeo_cluster, k=5, border = 2:5)
```
...
nigeria_sf_ngeo_cluster %>% 
  st_set_geometry(NULL) %>%
  group_by(CLUSTER) %>%
  summarise(mean_functional = mean(wpt.functional),
            mean_non_functional = mean(wpt.non.functional),
...
The `as.dist()` function is used to convert the data frame to a matrix. `choicealpha()` is then used to determine an appropriate value for the mixing parameter alpha.
...
#| code-fold: true
#| code-summary: "Show the code"
cg.map <- qtm(nigeria_sf_Gcluster, "CLUSTER") + 
  tm_borders(alpha = 0.5) +tm_layout(main.title = "ClustGeo method", 
                                      main.title.position = "center",
                                      main.title.size = 0.7,
                                      main.title.fontface = "bold",
                                      legend.height = 0.45, 
...
#### 10.4.1 Visualising individual clustering variables
...
#| code-fold: true
#| code-summary: "Show the code"
ggplot(data=nigeria_sf_Gcluster,
       aes(X=CLUSTER, y=pct_functional)) + geom_boxplot()
```

As we can see, **cluster 4** has the highest mean percentage of functional water points, whilst **cluster 1** has the lowest mean percentage of functional water points.

#### 10.4.2 Plotting the cluster summary 

We can also come up with the cluster summary plot here.

```{r}
#| code-fold: true
#| code-summary: "Show the code"
cluster_summary_clustGeo <- nigeria_sf_Gcluster %>% st_set_geometry(NULL) %>% group_by(CLUSTER) %>% summarise(pct_functional, pct_non_functional, pct_hand_pump, pct_usage_at_least_1000, pct_rural)
...
#| code-fold: true
#| code-summary: "Show the code"
cluster_summary_clustGeo.molten <- melt(cluster_summary_clustGeo, na.rm=TRUE)
...
#| code-fold: true
#| code-summary: "Show the code"
cluster_summary_plot_clustGeo <-
  ggplot(cluster_summary_clustGeo.molten, aes(variable, value, fill = variable)) + 
  facet_wrap(~ CLUSTER) +
  geom_bar(stat="identity", show_guide=FALSE) + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
#### 10.4.3 Multivariate Visualisation
...
#| code-fold: true
#| code-summary: "Show the code"
ggparcoord(data = cluster_summary_clustGeo, 
           columns = c(2:6), 
           scale = "globalminmax",
           alphaLines = 0.2,
           boxplot = TRUE, 
...
We can see from the outputs above that, **cluster 4** has relatively well-maintained water points. It has the highest mean percentage of functional water points and hand pumps, and also lowest mean percentage for its non-functional water points and percentage usage.

**Clusters 2 and 5** also have relatively high mean percentage of functional water points as well. They also scored relatively low in mean percentage of non-functional water points, which suggests that these 2 clusters do not have much issues with its water points.

**Clusters 1 and 3** fared the lowest in mean percentage of functional water points. They also have higher mean percentage of non-functional water points in comparison to the other clusters. These may be areas of concerns where the government may want to look into to improve the functionality of the water points.

## 11. Conclusion

Now, let's take a final look at the choropleth maps that have been mapped using hierarchical clustering method, k-means method, SKATER method and ClustGeo method.

```{r message=FALSE, warning=FALSE}
#| code-fold: true
#| code-summary: "Show the code"
tmap_arrange(hclust.map, km.map, shclust.map, cg.map,
             asp=1, ncol=2)
```

As we can see, all the methods above have its benefits and limitations of performing cluster analysis. With a side by side comparison, it can be seen that the conventional hierarchical clustering method, the k-means clustering method, as well as the ClustGeo method, produced relatively similar looking choropleth maps. The SKATER method did not produce a similar map than as compared to the other maps and clearly, it may be less wise to use the SKATER method for our cluster analysis here even though they appear less fragmented here. This is because the LGAs may not be spatially contiguous based on the variables that we are analysing in this exercise.

## 12. References

Jordan, O. Z. R. (November 2022). *Bringing Data to Life - Geospatial Analytics for Social Good - Understanding Nigeria Water functional and non-functional water point rate*. https://jordan-isss624-geospatial.netlify.app/posts/geo/geospatial_exercise/

Kam. (2022, December 6). *R for Geospatial Data Science and Analytics - 5  Geographical Segmentation with Spatially Constrained Clustering Techniques*. https://r4gdsa.netlify.app/chap05.html
